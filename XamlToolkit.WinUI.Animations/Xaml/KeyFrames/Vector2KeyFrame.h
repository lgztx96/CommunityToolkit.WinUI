...
{
    struct Vector2KeyFrame : Vector2KeyFrameT<Vector2KeyFrame, KeyFrameBase<winrt::hstring, winrt::float2>>
    {
    public:
    ...
    protected:
        std::optional<winrt::float2> GetParsedValue() const override
        {
            auto value = Value();
            if (value.empty())
            {
