...
namespace winrt::CommunityToolkit::WinUI::implementation
{
	...
	void AttachedShadowBase::ConnectElement(FrameworkElement const& element)
	{
		...
		auto iter = std::find_if(ShadowElementContextTable.begin(), ShadowElementContextTable.end(), [=](auto&& ctx)
			{
				return ctx.Element() == element;
			});

		if (iter == ShadowElementContextTable.end())
		{
			auto context = winrt::make<CommunityToolkit::WinUI::implementation::AttachedShadowElementContext>(*this, element);
			ShadowElementContextTable.emplace_back(context);
		}
	}
	...
	{
		auto iter = std::find_if(ShadowElementContextTable.begin(), ShadowElementContextTable.end(), [=](auto&& v)
			{
	...
	{
		OnPropertyChanged(context, OpacityProperty, winrt::box_value(Opacity()), winrt::box_value(Opacity()));
		OnPropertyChanged(context, BlurRadiusProperty, winrt::box_value(BlurRadius()), winrt::box_value(BlurRadius()));
		OnPropertyChanged(context, ColorProperty, winrt::box_value(Color()), winrt::box_value(Color()));
		OnPropertyChanged(context, OffsetProperty, winrt::box_value(Offset()), winrt::box_value(Offset()));
	...
	{
		std::vector<CommunityToolkit::WinUI::AttachedShadowElementContext> vec{ ShadowElementContextTable };
	...
	{
		for (const auto& item : ShadowElementContextTable)
		{
			if (item && item.IsInitialized())
			{
				overridable().OnPropertyChanged(item, property, oldValue, newValue);
			}
		}
	}
